...
void main()
{
    ...
    //get SSAO ambient occlusion
    float ambientOcclusion = texelFetch(uGSSAO, ivec2(gl_FragCoord.xy), 0).r;
    //vec3 ka = (0.5 * ka * ambientOcclusion);
    	ka*= ambientOcclusion;
...
//fColor = vec3(texelFetch(uGSSAO, ivec2(gl_FragCoord.xy), 0)) ;
    //fColor = vec3(ambientOcclusion, ambientOcclusion, ambientOcclusion);
    fColor = ka;
