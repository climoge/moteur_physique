...
{	
    ...
    auto ellapsedTime = glfwGetTime();
    auto seconds = glfwGetTime();
    
    double h = 1.f/FREQUENCY;
    ...
    {			
        ...
        if(ellapsedTime > h){
			...
			//std::cout << "Ellapsed time : " << ellapsedTime << std::endl;
			
			// Computing particules
			...
			for(auto currentLink = links.begin(); currentLink != links.end(); ++currentLink)
			{
				...
				currentLink->LinkGravite(glm::vec3(0.f, -9.8f, 0.f));
				count++;
			}
			
			for(auto currentPMat = pMats.begin(); currentPMat != pMats.end(); ++currentPMat){
				currentPMat->UpdateLeapFrog(h);
			}
			...
			{
				//const auto modelMatrix = glm::rotate(glm::mat4(1), 0.2f * float(seconds), glm::vec3(0, 1, 0));
				const auto modelMatrix = glm::scale(glm::mat4(1), glm::vec3(10.f/SIZE, 10.f/SIZE, 10.f/SIZE));
...

{
    ...
    double alpha = 0.2f;
    double k = alpha * FREQUENCY * FREQUENCY; // Raideur
	double z = (alpha / 20.f) * FREQUENCY; // Viscosite
    ...
    for(float j = 0; j < SIZE; j += jOffset*2){
		for(float i = 0; i < SIZE; i += iOffset*2){
			/*pMats.push_back(PMat(glm::vec3(i, j, 0), 1.f, false));
			
			if(index == (m_width * m_height) - (2 * m_width)) pMats.push_back(PMat(glm::vec3(i, j + jOffset, 0), 1.f, true)); // Anchor point		
			else pMats.push_back(PMat(glm::vec3(i, j + jOffset, 0), 1.f, false));
			
			pMats.push_back(PMat(glm::vec3(i + iOffset, j, 0), 1.f, false));
			
			if(index == (m_height*m_width)-4) pMats.push_back(PMat(glm::vec3(i + iOffset, j + jOffset, 0), 1.f, true));  // Anchor point
			else pMats.push_back(PMat(glm::vec3(i + iOffset, j + jOffset, 0), 1.f, false));*/
			
			pMats.push_back(PMat(glm::vec3(i, 0, j), 1.f, false));
			
			if(index == (m_width * m_height) - (2 * m_width)) pMats.push_back(PMat(glm::vec3(i, 0, j + jOffset), 1.f, true)); // Anchor point		
			else pMats.push_back(PMat(glm::vec3(i, 0, j + jOffset), 1.f, false));
			
			pMats.push_back(PMat(glm::vec3(i + iOffset, 0, j), 1.f, false));
			
			if(index == (m_height*m_width)-4) pMats.push_back(PMat(glm::vec3(i + iOffset, 0, j + jOffset), 1.f, true));  // Anchor point
			else pMats.push_back(PMat(glm::vec3(i + iOffset, 0, j + jOffset), 1.f, false));
    ...
    m_sphereGeometry = glmlv::makeSphere(20);
